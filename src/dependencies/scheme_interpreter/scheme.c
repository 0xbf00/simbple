...
static const char *strlwr(char *s) { return s; }
#endif
...
#define is_outport(p)                                                          \
  (type(p) == T_PORT && p->_object._port->kind & port_output)
...
INTERFACE INLINE int is_blob(pointer p) { return (type(p) == T_BLOB); }
INTERFACE INLINE blob_info_t blob_info(pointer p) { return (p)->_object._bi; }
...
INTERFACE pointer mk_blob(scheme *sc, size_t tag, void *blob, size_t size,
                          blob_free_func blob_free) {
  pointer x = get_cell(sc, sc->NIL, sc->NIL);

  typeflag(x) = (T_BLOB | T_ATOM);
  x->_object._bi.tag = tag;
  x->_object._bi.blob = blob;
  x->_object._bi.size = size;
  x->_object._bi.blob_free = blob_free;
...
static void finalize_cell(scheme *sc, pointer a) {
  if (is_string(a)) {
    ...
    blob_info_t bi = blob_info(a);
    if (bi.blob_free) {
      printf("Freeing object at %p\n", a);
      bi.blob_free(bi.blob, bi.tag);
    } else {
      printf("Cannot free blob, as no free function supplied.\n");
    }
  } else if (is_port(a)) {
...
static INLINE pointer slot_value_in_env(pointer slot) { return cdr(slot); }
...
static INLINE void dump_stack_reset(scheme *sc) { sc->dump = sc->NIL; }

static INLINE void dump_stack_initialize(scheme *sc) { dump_stack_reset(sc); }
...
static INLINE void dump_stack_mark(scheme *sc) { mark(sc->dump); }
#endif
...
static pointer opexe_0(scheme *sc, enum scheme_opcodes op) {
  ...
  case OP_LET2: /* let */
    ...
    if (is_symbol(car(sc->code))) { /* named let */
      ...
      x = mk_closure(
          sc, cons(sc, reverse_in_place(sc, sc->NIL, sc->args), cddr(sc->code)),
          sc->envir);
...
static void Eval_Cycle(scheme *sc, enum scheme_opcodes op) {
  ...
  for (;;) {
    ...
    if (pcd->name != 0) { /* if built-in function, check arguments */
      ...
      if (n < pcd->min_arity) {
        ok = 0;
        sprintf(msg, "%s: needs%s %d argument(s)", pcd->name,
      ...
      if (ok) {
        if (pcd->arg_tests_encoding != 0) {
          ...
          if (i < n) {
            ok = 0;
            sprintf(msg, "%s: argument %d must be: %s", pcd->name, i + 1,
                    tests[j].kind);
          }
        }
      }
...
static struct scheme_interface vtbl = {scheme_define,    s_cons,
                                       s_immutable_cons, reserve_cells,
                                       mk_integer,       mk_real,
                                       mk_symbol,        gensym,
                                       mk_string,        mk_counted_string,
                                       mk_character,     mk_vector,
                                       mk_foreign_func,  mk_blob,
                                       putstr,           putcharacter,

                                       is_string,        string_value,
                                       is_number,        nvalue,
                                       ivalue,           rvalue,
                                       is_integer,       is_real,
                                       is_character,     charvalue,
                                       is_vector,        ivalue,
                                       fill_vector,      vector_elem,
                                       set_vector_elem,  is_port,
                                       is_pair,          pair_car,
                                       pair_cdr,         set_car,
                                       set_cdr,
                                       ...
                                       is_syntax,        is_proc,
                                       is_foreign,       is_blob,
                                       blob_info,        syntaxname,
                                       is_closure,       is_macro,
                                       closure_code,     closure_env,

                                       is_continuation,  is_promise,
                                       is_environment,   is_immutable,
                                       setimmutable,

                                       scheme_load_file, scheme_load_string};
#endif
